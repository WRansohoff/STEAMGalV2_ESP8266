...
void draw_esp_text_outlines(void) {
  ...
  oled_draw_rect(0, 0, 96, 64, 2, 3);
  oled_draw_h_line(2, 12, 92, 3);
  oled_draw_h_line(2, 23, 92, 3);
  oled_draw_v_line(10, 13, 49, 3);
  // ('Up' caret)
  oled_draw_v_line(18, 13, 10, 3);
  oled_draw_rect(12, 14, 5, 3, 0, 8);
  oled_draw_rect(13, 17, 3, 3, 0, 8);
  oled_draw_rect(14, 20, 1, 2, 0, 8);
  // ('Down' caret)
  oled_draw_v_line(86, 13, 10, 3);
  ...
  // Draw the currently-selected "AT" command in the
  // "Transmit" row.
  redraw_at_cmd();
...
void redraw_at_cmd(void) {
  oled_draw_rect(19, 13, 67, 10, 0, 0);
  oled_draw_text(20, 14, (char*)at_commands[selected_at_cmd], 8, 'S');
  should_refresh_display = 1;
}

void redraw_trx_state(int x, char* state) {
  oled_draw_rect(2, 2, 92, 10, 0, 0);
  oled_draw_text(x, 3, state, 7, 'S');
  should_refresh_display = 1;
}
